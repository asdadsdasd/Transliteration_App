...
* \param [in] fileName - Имя файла, в котором храниться исходная строка
* \return Введенныую пользователем строку
* \note Если файл открыть не удаось, то приводит к остановке программы
*/
string inputFile(string fileName);

/*!
* Функция выполняет транслит переданной строки всеми возможными вариантами
* \param [in] eng - введенная пользователем строка
* \return Строку после транслитерации на русский язык
*/
vector<string> translit(string eng);

/*!
* Функция рекурсивно выполняет транслит строки
...
* \return Строку символов в верхнем регистре
*/
...
* \param [in] line - строка с символами
* \return Строку символов в нижнем регистре
*/
